...
DSPy optimization (MIPROv2 `auto=medium`, Claude Haiku 4.5), measured by 5-fold cross-validation on 100 hand-labelled queries (10 per intent) — each query scored by a fold model that never trained on it, so no train/test contamination — lifts held-out intent accuracy from **91.0% → 99.0%** (**+8.0 ± 4.5 pp**, every fold improved). No hand-authored prompt: the optimizer selects instructions and few-shot demonstrations against the accuracy metric. Reproduce with `python backend/scripts/crossval_intent_classifier.py --llm haiku`. Caveats: single-annotator labels reflecting the author's anticipated query distribution (not production traffic); the compiled program adds per-query latency over the zero-shot path.
